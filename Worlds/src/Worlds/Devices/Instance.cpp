...
CallbackDebug(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
    ...
    if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
        W_WARN(pCallbackData->pMessage, '\n');
    else if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT)
        W_INFO(pCallbackData->pMessage, '\n');
    else // VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT
        W_ERROR(pCallbackData->pMessage, '\n');

    return VK_FALSE;
}

VkResult Instance::FvkCreateDebugUtilsMessengerEXT(
...
void Instance::FvkDestroyDebugUtilsMessengerEXT(
...
VKAPI_ATTR VkBool32 VKAPI_CALL CallbackDebug(
...
VkResult Instance::FvkCreateDebugReportCallbackEXT(
...
void Instance::FvkDestroyDebugReportCallbackEXT(
...
void Instance::FvkCmdPushDescriptorSetKHR(
    ...
    if (func)
        func(commandBuffer, pipelineBindPoint, layout, set,
...
bool Instance::CheckValidationLayerSupport() const {
    ...
    for (const auto &layerName : ValidationLayers) {
        ...
        if (!layerFound) {
            W_ERROR("Vulkan validation layer not found: ",
                    std::quoted(layerName), '\n');
            return false;
        }
    }
...
void Instance::CreateInstance() {
    ...
    // applicationInfo.applicationVersion = VK_MAKE_VERSION(appVersion.major,
    // appVersion.minor, appVersion.patch);
    applicationInfo.pEngineName = "Acid";
    applicationInfo.engineVersion = VK_MAKE_VERSION(0, 0, 0);
    applicationInfo.apiVersion = volkGetInstanceVersion() >= VK_API_VERSION_1_1
                                     ? VK_API_VERSION_1_1
                                     : VK_MAKE_VERSION(1, 0, 57);

    if (enableValidationLayers && !CheckValidationLayerSupport()) {
        W_ERROR("Validation layers requested, but not available!\n");
...
void Instance::CreateDebugMessenger() {
    ...
    auto debugReportResult = FvkCreateDebugReportCallbackEXT(
        instance, &debugReportCallbackCreateInfo, nullptr,
        &debugReportCallback);
    if (debugReportResult == VK_ERROR_EXTENSION_NOT_PRESENT) {
        enableValidationLayers = false;
        W_ERROR("Extension vkCreateDebugReportCallbackEXT not present!\n");
    } else {
