...
LogicalDevice::LogicalDevice(const Instance *instance,
    ...
    : instance(instance), physicalDevice(physicalDevice), surface(surface) {
    CreateQueueIndices();
    CreateLogicalDevice();
}
...
void LogicalDevice::CreateQueueIndices() {
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(
        *physicalDevice, &deviceQueueFamilyPropertyCount, nullptr);
    std::vector<VkQueueFamilyProperties> deviceQueueFamilyProperties(
        deviceQueueFamilyPropertyCount);
    vkGetPhysicalDeviceQueueFamilyProperties(
        *physicalDevice, &deviceQueueFamilyPropertyCount,
        deviceQueueFamilyProperties.data());

    std::optional<uint32_t> graphicsFamily, presentFamily, computeFamily,
        transferFamily;
...
void LogicalDevice::CreateLogicalDevice() {
    ...
    if (supportedQueues & VK_QUEUE_TRANSFER_BIT &&
        transferFamily != graphicsFamily && transferFamily != computeFamily) {
    ...
    if (instance->GetEnableValidationLayers()) {
        ...
        deviceCreateInfo.ppEnabledLayerNames =
            Instance::ValidationLayers.data();
    }
    deviceCreateInfo.enabledExtensionCount =
        static_cast<uint32_t>(DeviceExtensions.size());
    deviceCreateInfo.ppEnabledExtensionNames = DeviceExtensions.data();
    deviceCreateInfo.pEnabledFeatures = &enabledFeatures;
    Graphics::CheckVk(vkCreateDevice(*physicalDevice, &deviceCreateInfo,
