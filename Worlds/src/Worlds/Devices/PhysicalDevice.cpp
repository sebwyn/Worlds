...
PhysicalDevice::PhysicalDevice(const Instance *instance) : instance(instance) {
    ...
    if (!physicalDevice)
        throw std::runtime_error(
            "Vulkan runtime error, failed to find a suitable GPU");
    ...
    W_INFO("Selected Physical Device: ", properties.deviceID, " ",
...
VkPhysicalDevice PhysicalDevice::ChoosePhysicalDevice(
    ...
    // Checks to make sure the best candidate scored higher than 0  rbegin
    // points to last element of ranked devices(highest rated), first is its
    // rating.
...
uint32_t PhysicalDevice::ScorePhysicalDevice(const VkPhysicalDevice &device) {
    ...
    vkEnumerateDeviceExtensionProperties(
        device, nullptr, &extensionPropertyCount, extensionProperties.data());
    ...
    // Obtain the device features and properties of the current device being
    // rateds.
...
void PhysicalDevice::LogVulkanDevice(
    ...
    ss << "API Version: " << supportedVersion[0] << "." << supportedVersion[1]
       << "." << supportedVersion[2] << '\n';
