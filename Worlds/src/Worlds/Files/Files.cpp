  ...
  private:
    int_type underflow() override {
        ...
        auto bytesRead = PHYSFS_readBytes(
            file, buffer, static_cast<PHYSFS_uint32>(bufferSize));
        if (bytesRead < 1)
            return traits_type::eof();
    ...
    int_type overflow(int_type c = traits_type::eof()) override {
        ...
        if (PHYSFS_writeBytes(file, pbase(),
                              static_cast<PHYSFS_uint32>(pptr() - pbase())) < 1)
            return traits_type::eof();
...
BaseFStream::BaseFStream(PHYSFS_File *file) : file(file) {
    if (file == NULL) {
        throw std::invalid_argument(
            "Attempted to construct fstream with a NULL file");
    }
}

BaseFStream::~BaseFStream() { PHYSFS_close(file); }

size_t BaseFStream::length() { return PHYSFS_fileLength(file); }

PHYSFS_File *OpenWithMode(const std::filesystem::path &filename,
                          FileMode openMode) {
...
IFStream::IFStream(const std::filesystem::path &filename)
    : BaseFStream(OpenWithMode(filename, FileMode::Read)), std::istream(
                                                               new FBuffer(
                                                                   file)) {}

IFStream::~IFStream() { delete rdbuf(); }

OFStream::OFStream(const std::filesystem::path &filename, FileMode writeMode)
    : BaseFStream(OpenWithMode(filename, writeMode)), std::ostream(
                                                          new FBuffer(file)) {}

OFStream::~OFStream() { delete rdbuf(); }

FStream::FStream(const std::filesystem::path &filename, FileMode openMode)
    : BaseFStream(OpenWithMode(filename, openMode)), std::iostream(
                                                         new FBuffer(file)) {}

FStream::~FStream() { delete rdbuf(); }

Files::Files() { PHYSFS_init(Application::Get().GetArgv0()); }
...
void Files::AddSearchPath(const std::string &path) {
    ...
    if (PHYSFS_mount(path.c_str(), nullptr, true) == 0) {
        W_WARN("Failed to mount path ", path, ", ",
...
void Files::RemoveSearchPath(const std::string &path) {
    ...
    if (PHYSFS_unmount(path.c_str()) == 0) {
        W_WARN("Failed to unmount path ", path, ", ",
                     PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode()), '\n');
        return;
    }
...
void Files::ClearSearchPath() {
    for (const auto &searchPath : searchPaths) {
        if (PHYSFS_unmount(searchPath.c_str()) == 0)
            W_WARN("Failed to unmount path ", searchPath, ", ",
                         PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode()),
                         '\n');
    }
...
std::optional<std::string> Files::Read(const std::filesystem::path &path) {
    ...
    if (!fsFile) {
        if (!std::filesystem::exists(path) ||
            ...
            W_ERROR("Failed to open file ", path, ", ",
                       PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode()), '\n');
            return std::nullopt;
        }
    ...
    if (PHYSFS_close(fsFile) == 0)
        W_ERROR("Failed to close file ", path, ", ",
                   PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode()), '\n');
...
std::vector<unsigned char> Files::ReadBytes(const std::filesystem::path &path) {
    ...
    std::copy(std::istream_iterator<unsigned char>(file),
              std::istream_iterator<unsigned char>(), back_inserter(bytes));

    return bytes;
}

std::vector<std::string> Files::FilesInPath(const std::filesystem::path &path,
                                            bool recursive) {
    auto pathStr = path.string();
    std::replace(pathStr.begin(), pathStr.end(), '\\', '/');
    ...
    for (auto i = rc; *i; i++) {
        /*if (IsDirectory(*i)) {
                if (recursive) {
                        ...
                        files.insert(result.end(), filesInFound.begin(),
        filesInFound.end());
                }
        } else {*/
