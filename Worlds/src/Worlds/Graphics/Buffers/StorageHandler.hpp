...
class StorageHandler {
  public:
    ...
    void Push(void *data, std::size_t size) {
        ...
        // If the buffer is already changed we can skip a memory comparison and
        // just copy.
        if (handlerStatus == Buffer::Status::Changed ||
            std::memcmp(static_cast<char *>(this->data), data, size) != 0) {
    ...
    void Push(const T &object, std::size_t offset, std::size_t size) {
        ...
        // just copy.
        if (handlerStatus == Buffer::Status::Changed ||
            std::memcmp(static_cast<char *>(this->data) + offset, &object,
                        size) != 0) {
            std::memcpy(static_cast<char *>(this->data) + offset, &object,
                        size);
            handlerStatus = Buffer::Status::Changed;
        }
    }
