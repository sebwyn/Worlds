...
CommandBuffer::CommandBuffer(bool begin, VkQueueFlagBits queueType,
    ...
    : commandPool(Graphics::Get()->GetCommandPool()), queueType(queueType) {
...
CommandBuffer::~CommandBuffer() {
    ...
    vkFreeCommandBuffers(*logicalDevice, commandPool->GetCommandPool(), 1,
                         &commandBuffer);
}
...
void CommandBuffer::SubmitIdle() {
    ...
    Graphics::CheckVk(vkWaitForFences(*logicalDevice, 1, &fence, VK_TRUE,
...
void CommandBuffer::Submit(const VkSemaphore &waitSemaphore,
                           const VkSemaphore &signalSemaphore, VkFence fence) {
    auto logicalDevice = Graphics::Get()->GetLogicalDevice();
    auto queueSelected = GetQueue();
