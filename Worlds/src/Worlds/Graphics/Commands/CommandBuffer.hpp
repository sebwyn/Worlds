...
class CommandBuffer {
  public:
     ...
     * @param begin If recording will start right away, if true {@link
     * CommandBuffer#Begin} is called.
    ...
    /**
     * Submits the command buffer to the queue and will hold the current thread
     * idle until it has finished.
     */
    void SubmitIdle();

    /**
     * Submits the command buffer.
     * @param waitSemaphore A optional semaphore that will waited upon before
     * the command buffer is executed.
     * @param signalSemaphore A optional that is signaled once the command
     * buffer has been executed.
     * @param fence A optional fence that is signaled once the command buffer
     * has completed.
     */
