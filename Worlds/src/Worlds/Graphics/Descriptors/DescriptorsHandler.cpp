...
DescriptorsHandler::DescriptorsHandler(const Pipeline &pipeline)
    : shader(pipeline.GetShader()),
      ...
      descriptorSet(std::make_unique<DescriptorSet>(pipeline)), changed(true) {}

void DescriptorsHandler::Push(const std::string &descriptorName,
    ...
    if (changed) {
        ...
        for (const auto &[descriptorName, descriptor] : descriptors) {
            auto writeDescriptorSet =
                descriptor.writeDescriptor.GetWriteDescriptorSet();
...
void DescriptorsHandler::BindDescriptor(const CommandBuffer &commandBuffer,
    ...
    if (pushDescriptors) {
        ...
        Instance::FvkCmdPushDescriptorSetKHR(
            *logicalDevice, commandBuffer, pipeline.GetPipelineBindPoint(),
