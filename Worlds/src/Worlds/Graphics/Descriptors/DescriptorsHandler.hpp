...
#include "Worlds/Graphics/Buffers/PushHandler.hpp"
#include "Worlds/Graphics/Buffers/StorageHandler.hpp"
#include "Worlds/Graphics/Buffers/UniformHandler.hpp"
#include "Worlds/Graphics/Descriptors/DescriptorSet.hpp"
...
class DescriptorsHandler {
  public:
    ...
    template <typename T>
    void Push(const std::string &descriptorName, const T &descriptor,
        ...
        if (it != descriptors.end()) {
            // If the descriptor and size have not changed then the write is not
            // modified.
            if (it->second.descriptor == to_address(descriptor) &&
                it->second.offsetSize == offsetSize) {
                return;
            }
        ...
        if (!location) {
            ...
            if (shader->ReportedNotFound(descriptorName, true)) {
                W_ERROR("Could not find descriptor in shader ",
                        shader->GetName(), " of name ",
        ...
        if (!descriptorType) {
            ...
            if (shader->ReportedNotFound(descriptorName, true)) {
                W_ERROR("Could not find descriptor in shader ",
                        shader->GetName(), " of name ",
                        std::quoted(descriptorName), " at location ", *location,
                        '\n');
            }
#endif
            return;
        ...
        auto writeDescriptor =
            to_address(descriptor)
                ->GetWriteDescriptor(*location, *descriptorType, offsetSize);
        descriptors.emplace(descriptorName,
                            DescriptorValue{to_address(descriptor),
    ...
    void Push(const std::string &descriptorName, const T &descriptor,
        ...
        if (auto it = descriptors.find(descriptorName);
            it != descriptors.end()) {
            descriptors.erase(it);
        }
