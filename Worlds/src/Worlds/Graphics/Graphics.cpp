...
Graphics::Graphics()
    : /*elapsedPurge(5s), */ instance(std::make_unique<Instance>()),
      ...
      logicalDevice(std::make_unique<LogicalDevice>(
          instance.get(), physicalDevice.get(), surface.get())) {
    s_instance = this;
...
void Graphics::Update() {
    if (!renderer || Application::Get().IsMinimized())
        return;
    ...
    auto acquireResult = swapchain->AcquireNextImage(
        presentCompletes[currentFrame], flightFences[currentFrame]);
    ...
    if (acquireResult != VK_SUCCESS && acquireResult != VK_SUBOPTIMAL_KHR) {
        W_ERROR("Failed to acquire swap chain image!\n");
        return;
    }
    ...
    for (auto &renderStage : renderer->renderStages) {
        ...
        for (const auto &subpass : renderStage->GetSubpasses()) {
            ...
            if (subpass.GetBinding() !=
                renderStage->GetSubpasses().back().GetBinding())
...
std::string Graphics::StringifyResultVk(VkResult result) {
    ...
    case VK_ERROR_NATIVE_WINDOW_IN_USE_KHR:
        return "The requested window is already connected to a VkSurfaceKHR, "
...
void Graphics::CaptureScreenshot(const std::filesystem::path &filename) const {
    ...
    auto supportsBlit =
        Image::CopyImage(swapchain->GetActiveImage(), dstImage, dstImageMemory,
                         surface->GetFormat().format, {size.x, size.y, 1},
...
#if defined(ACID_DEBUG)
    W_INFO("Screenshot ", filename, " created in ",
...
Graphics::GetCommandPool(const std::thread::id &threadId) {
    if (auto it = commandPools.find(threadId); it != commandPools.end())
        return it->second;
    // TODO: Cleanup and fix crashes
    return commandPools
        .emplace(threadId, std::make_shared<CommandPool>(threadId))
        .first->second;
}
...
void Graphics::CreatePipelineCache() {
    ...
    CheckVk(vkCreatePipelineCache(*logicalDevice, &pipelineCacheCreateInfo,
...
void Graphics::RecreateCommandBuffers() {
    ...
    for (std::size_t i = 0; i < flightFences.size(); i++) {
        CheckVk(vkCreateSemaphore(*logicalDevice, &semaphoreCreateInfo, nullptr,
                                  &presentCompletes[i]));

        CheckVk(vkCreateSemaphore(*logicalDevice, &semaphoreCreateInfo, nullptr,
                                  &renderCompletes[i]));

        CheckVk(vkCreateFence(*logicalDevice, &fenceCreateInfo, nullptr,
                              &flightFences[i]));
...
void Graphics::RecreatePass(RenderStage &renderStage) {
    ...
    if (renderStage.HasSwapchain() &&
        (framebufferResized || !swapchain->IsSameExtent(displayExtent)))
        RecreateSwapchain();
...
void Graphics::RecreateAttachmentsMap() {
    ...
    for (const auto &renderStage : renderer->renderStages)
        attachments.insert(renderStage->descriptors.begin(),
                           renderStage->descriptors.end());
}
...
bool Graphics::StartRenderpass(RenderStage &renderStage) {
    ...
    VkRect2D renderArea = {};
    renderArea.offset = {renderStage.GetRenderArea().GetOffset().x,
                         renderStage.GetRenderArea().GetOffset().y};
    renderArea.extent = {renderStage.GetRenderArea().GetExtent().x,
                         renderStage.GetRenderArea().GetExtent().y};
...
void Graphics::EndRenderpass(RenderStage &renderStage) {
    ...
    commandBuffer->End();
    commandBuffer->Submit(presentCompletes[currentFrame],
                          renderCompletes[currentFrame],
                          flightFences[currentFrame]);

    auto presentResult =
        swapchain->QueuePresent(presentQueue, renderCompletes[currentFrame]);
    if (presentResult == VK_ERROR_OUT_OF_DATE_KHR ||
        ...
        W_ERROR("Failed to present swap chain image!\n");
    }
