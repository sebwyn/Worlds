...
Image::Image(VkFilter filter, VkSamplerAddressMode addressMode,
    ...
    : extent(extent), samples(samples), usage(usage), format(format),
      mipLevels(mipLevels), arrayLayers(arrayLayers), filter(filter),
      addressMode(addressMode), layout(layout) {}
...
WriteDescriptorSet
Image::GetWriteDescriptor(uint32_t binding, VkDescriptorType descriptorType,
    ...
    descriptorWrite.dstSet =
        VK_NULL_HANDLE; // Will be set in the descriptor handler.
...
Image::GetDescriptorSetLayout(uint32_t binding, VkDescriptorType descriptorType,
...
std::unique_ptr<Bitmap> Image::GetBitmap(uint32_t mipLevel,
                                         uint32_t arrayLayer) const {
    auto logicalDevice = Graphics::Get()->GetLogicalDevice();

    glm::uvec2 size(int32_t(extent.width >> mipLevel),
                    int32_t(extent.height >> mipLevel));

    VkImage dstImage;
    VkDeviceMemory dstImageMemory;
    CopyImage(image, dstImage, dstImageMemory, format, {size.x, size.y, 1},
              layout, mipLevel, arrayLayer);
    ...
    auto bitmap = std::make_unique<Bitmap>(
        std::make_unique<uint8_t[]>(dstSubresourceLayout.size), size);
...
void Image::CreateImage(VkImage &image, VkDeviceMemory &memory,
    ...
    imageCreateInfo.flags =
        arrayLayers == 6 ? VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT : 0;
    ...
    Graphics::CheckVk(vkAllocateMemory(*logicalDevice, &memoryAllocateInfo,
                                       nullptr, &memory));
...
void Image::CreateImageView(const VkImage &image, VkImageView &imageView,
    ...
    Graphics::CheckVk(vkCreateImageView(*logicalDevice, &imageViewCreateInfo,
                                        nullptr, &imageView));
}
...
void Image::CreateMipmaps(const VkImage &image, const VkExtent3D &extent,
    ...
    for (uint32_t i = 1; i < mipLevels; i++) {
        ...
        imageBlit.dstOffsets[1] = {int32_t(extent.width >> i),
...
void Image::InsertImageMemoryBarrier(
    const CommandBuffer &commandBuffer, const VkImage &image,
...
bool Image::CopyImage(const VkImage &srcImage, VkImage &dstImage,
                      VkDeviceMemory &dstImageMemory, VkFormat srcFormat,
    ...
    vkGetPhysicalDeviceFormatProperties(
        *physicalDevice, surface->GetFormat().format, &formatProperties);

    if (!(formatProperties.optimalTilingFeatures &
          VK_FORMAT_FEATURE_BLIT_SRC_BIT)) {
        W_WARN("Device does not support blitting from optimal tiled "
               "images, using copy instead of blit!\n");
        supportsBlit = false;
    }

    // Check if the device supports blitting to linear images.
    vkGetPhysicalDeviceFormatProperties(*physicalDevice, srcFormat,
                                        &formatProperties);

    if (!(formatProperties.linearTilingFeatures &
          VK_FORMAT_FEATURE_BLIT_DST_BIT)) {
        W_WARN("Device does not support blitting to linear tiled images, "
               "using copy instead of blit!\n");
        supportsBlit = false;
    }

    CreateImage(dstImage, dstImageMemory, extent, VK_FORMAT_R8G8B8A8_UNORM,
    ...
    InsertImageMemoryBarrier(
        commandBuffer, srcImage, VK_ACCESS_MEMORY_READ_BIT,
        VK_ACCESS_TRANSFER_READ_BIT, srcImageLayout,
        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, mipLevel,
        1, arrayLayer);
    ...
    InsertImageMemoryBarrier(
        commandBuffer, srcImage, VK_ACCESS_TRANSFER_READ_BIT,
        VK_ACCESS_MEMORY_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        srcImageLayout, VK_PIPELINE_STAGE_TRANSFER_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, mipLevel,
        1, arrayLayer);
