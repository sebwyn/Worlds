...
class Image : public Descriptor {
  public:
     ...
     * @param format The format and type of the texel blocks that will be
     * contained in the image.
     * @param mipLevels The number of levels of detail available for minified
     * sampling of the image.
     * @param arrayLayers The number of layers in the image.
     * @param extent The number of data elements in each dimension of the base
     * level.
     */
    ...
    /**
     * Copies the images pixels from memory to a bitmap. If this method is
     * called from multiple threads at the same time Vulkan will crash!
     ...
     */
    std::unique_ptr<Bitmap> GetBitmap(uint32_t mipLevel = 0,
    ...
    /**
     * Find a format in the candidates list that fits the tiling and features
     * required.
     * @param candidates Formats that are tested for features, in order of
     * preference.
    ...
    static void CopyBufferToImage(const VkBuffer &buffer, const VkImage &image,
                                  ...
                                  uint32_t baseArrayLayer);
    static bool CopyImage(const VkImage &srcImage, VkImage &dstImage,
                          VkDeviceMemory &dstImageMemory, VkFormat srcFormat,
                          const VkExtent3D &extent,
                          VkImageLayout srcImageLayout, uint32_t mipLevel,
                          uint32_t arrayLayer);
