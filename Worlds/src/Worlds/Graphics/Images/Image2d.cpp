...
namespace Worlds {
Image2d::Image2d(std::filesystem::path filename, VkFilter filter,
...
Image2d::Image2d(const glm::uvec2 &extent, VkFormat format,
...
Image2d::Image2d(std::unique_ptr<Bitmap> &&bitmap, VkFormat format,
                 VkImageLayout layout, VkImageUsageFlags usage, VkFilter filter,
                 VkSamplerAddressMode addressMode,
                 VkSampleCountFlagBits samples, bool anisotropic, bool mipmap)
    : Image(filter, addressMode, samples, layout,
...
void Image2d::SetPixels(const uint8_t *pixels, uint32_t layerCount,
...
void Image2d::Load(std::unique_ptr<Bitmap> loadBitmap) {
    ...
    CreateImageView(image, view, VK_IMAGE_VIEW_TYPE_2D, format,
