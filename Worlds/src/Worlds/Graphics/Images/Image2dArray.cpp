#include "Worlds/Graphics/Images/Image2dArray.hpp"

#include "Worlds/Bitmaps/Bitmap.hpp"
#include "Worlds/Graphics/Buffers/Buffer.hpp"

namespace Worlds {
Image2dArray::Image2dArray(const glm::uvec2 &extent, uint32_t arrayLayers,
    ...
    CreateImage(image, memory, this->extent, format, samples,
                VK_IMAGE_TILING_OPTIMAL, this->usage,
    ...
    CreateImageView(image, view, VK_IMAGE_VIEW_TYPE_2D_ARRAY, format,
                    VK_IMAGE_ASPECT_COLOR_BIT, mipLevels, 0, arrayLayers, 0);
    TransitionImageLayout(image, format, VK_IMAGE_LAYOUT_UNDEFINED, layout,
                          VK_IMAGE_ASPECT_COLOR_BIT, mipLevels, 0, arrayLayers,
                          0);
}

Image2dArray::Image2dArray(std::unique_ptr<Bitmap> &&bitmap,
    ...
    CreateImage(image, memory, extent, format, samples, VK_IMAGE_TILING_OPTIMAL,
                this->usage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mipLevels,
    ...
    TransitionImageLayout(image, format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                          layout, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels, 0,
                          arrayLayers, 0);
}
