#include "Worlds/Graphics/Pipelines/PipelineCompute.hpp"

#include "Worlds/Files/Files.hpp"
#include "Worlds/Graphics/Graphics.hpp"

namespace Worlds {
PipelineCompute::PipelineCompute(std::filesystem::path shaderStage,
...
#if defined(ACID_DEBUG)
    Log::Out("Pipeline Compute ", this->shaderStage, " created in ",
...
void PipelineCompute::CmdRender(const CommandBuffer &commandBuffer,
                                const glm::uvec2 &extent) const {
    auto groupCountX = static_cast<uint32_t>(
        std::ceil(static_cast<float>(extent.x) /
                  static_cast<float>(*shader->GetLocalSizes()[0])));
    auto groupCountY = static_cast<uint32_t>(
        std::ceil(static_cast<float>(extent.y) /
                  static_cast<float>(*shader->GetLocalSizes()[1])));
...
void PipelineCompute::CreateShaderProgram() {
    ...
    if (!fileLoaded)
        throw std::runtime_error(
            "Could not create compute pipeline, missing shader stage");

    auto stageFlag = Shader::GetShaderStage(shaderStage);
    shaderModule = shader->CreateShaderModule(shaderStage, *fileLoaded,
                                              defineBlock.str(), stageFlag);
...
void PipelineCompute::CreatePipelineCompute() {
    ...
    vkCreateComputePipelines(*logicalDevice, pipelineCache, 1,
