...
PipelineGraphics::PipelineGraphics(
    Stage stage, std::vector<std::filesystem::path> shaderStages,
    ...
    VkCullModeFlags cullMode, VkFrontFace frontFace, bool pushDescriptors)
    : stage(std::move(stage)), shaderStages(std::move(shaderStages)),
      vertexInputs(std::move(vertexInputs)), defines(std::move(defines)),
      mode(mode), depth(depth), topology(topology), polygonMode(polygonMode),
      cullMode(cullMode), frontFace(frontFace),
      pushDescriptors(pushDescriptors), shader(std::make_unique<Shader>()),
...
#if defined(ACID_DEBUG)
    Log::Out("Pipeline Graphics ", this->shaderStages.back(), " loaded in ",
...
const ImageDepth *
PipelineGraphics::GetDepthStencil(const std::optional<uint32_t> &stage) const {
    return Graphics::Get()
...
const Image2d *
PipelineGraphics::GetImage(uint32_t index,
...
RenderArea
PipelineGraphics::GetRenderArea(const std::optional<uint32_t> &stage) const {
    return Graphics::Get()
        ->GetRenderStage(stage ? *stage : this->stage.first)
        ->GetRenderArea();
}
    ...
    for (const auto &shaderStage : shaderStages) {
        ...
        if (!fileLoaded)
            throw std::runtime_error(
                "Could not create pipeline, missing shader stage");

        auto stageFlag = Shader::GetShaderStage(shaderStage);
        auto shaderModule = shader->CreateShaderModule(
            shaderStage, *fileLoaded, defineBlock.str(), stageFlag);
...
void PipelineGraphics::CreateDescriptorLayout() {
    ...
    Graphics::CheckVk(vkCreateDescriptorSetLayout(
        *logicalDevice, &descriptorSetLayoutCreateInfo, nullptr,
        &descriptorSetLayout));
}
...
void PipelineGraphics::CreateDescriptorPool() {
    ...
    descriptorPoolCreateInfo.poolSizeCount =
        static_cast<uint32_t>(descriptorPools.size());
    descriptorPoolCreateInfo.pPoolSizes = descriptorPools.data();
    Graphics::CheckVk(vkCreateDescriptorPool(
        *logicalDevice, &descriptorPoolCreateInfo, nullptr, &descriptorPool));
}
...
void PipelineGraphics::CreatePipelineLayout() {
    ...
    Graphics::CheckVk(vkCreatePipelineLayout(
...
void PipelineGraphics::CreateAttributes() {
    ...
    if (polygonMode == VK_POLYGON_MODE_LINE &&
        ...
        throw std::runtime_error(
            "Cannot create graphics pipeline with line polygon mode when "
            "logical device does not support non solid fills.");
    }
    ...
    multisampleState.rasterizationSamples =
        multisampled ? physicalDevice->GetMsaaSamples() : VK_SAMPLE_COUNT_1_BIT;
...
void PipelineGraphics::CreatePipeline() {
    ...
    for (const auto &vertexInput : vertexInputs) {
        ...
        for (const auto &attribute : vertexInput.GetAttributeDescriptions()) {
            ...
            for (const auto &[shaderAttributeName, shaderAttribute] :
            shader->GetAttributes()) { if (attribute.location + lastAttribute ==
            shaderAttribute->GetLocation()) { shaderContains = true; break;
                    }
            }
    ...
    Graphics::CheckVk(vkCreateGraphicsPipelines(*logicalDevice, pipelineCache,
