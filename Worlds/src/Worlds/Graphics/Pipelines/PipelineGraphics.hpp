...
class PipelineGraphics : public Pipeline {
  public:
     ...
     * @param vertexInputs The vertex inputs that will be used as a shaders
     * input.
     ...
     * @param pushDescriptors If no actual descriptor sets are allocated but
     * instead pushed.
     */
    PipelineGraphics(
     ...
     * @param stage The stage to get values from, if not provided the pipelines
     * stage will be used.
     * @return The depth stencil that is found.
     */
    const ImageDepth *
    GetDepthStencil(const std::optional<uint32_t> &stage = std::nullopt) const;
     ...
     */
    RenderArea
    GetRenderArea(const std::optional<uint32_t> &stage = std::nullopt) const;

    const Stage &GetStage() const { return stage; }
    const std::vector<std::filesystem::path> &GetShaderStages() const {
        return shaderStages;
    }
...
class PipelineGraphicsCreate {
  public:
    PipelineGraphicsCreate(
        ...
        std::vector<Shader::VertexInput> vertexInputs = {},
        std::vector<Shader::Define> defines = {},
        PipelineGraphics::Mode mode = PipelineGraphics::Mode::Polygon,
        ...
        : shaderStages(std::move(shaderStages)),
          ...
          mode(mode), depth(depth), topology(topology),
          polygonMode(polygonMode), cullMode(cullMode), frontFace(frontFace),
    ...
    PipelineGraphics *Create(const Pipeline::Stage &pipelineStage) const {
        return new PipelineGraphics(pipelineStage, shaderStages, vertexInputs,
                                    defines, mode, depth, topology, polygonMode,
                                    cullMode, frontFace, pushDescriptors);
    }
