...
#include "Worlds/Graphics/Buffers/StorageBuffer.hpp"
#include "Worlds/Graphics/Buffers/UniformBuffer.hpp"
...
#include "Worlds/Utils/String.hpp"
...
class ShaderIncluder : public glslang::TShader::Includer {
  public:
    IncludeResult *includeLocal(const char *headerName,
        ...
        if (!fileLoaded) {
            W_ERROR("Shader Include could not be loaded: ",
                    std::quoted(headerName), '\n');
            return nullptr;
        }

        auto content = new char[fileLoaded->size()];
        std::memcpy(content, fileLoaded->c_str(), fileLoaded->size());
        return new IncludeResult(headerName, content, fileLoaded->size(),
                                 content);
    }
...
bool Shader::ReportedNotFound(const std::string &name,
                              bool reportIfFound) const {
    if (std::find(notFoundNames.begin(), notFoundNames.end(), name) ==
        notFoundNames.end()) {
        if (reportIfFound) {
...
Shader::GetDescriptorLocation(const std::string &name) const {
    if (auto it = descriptorLocations.find(name);
        it != descriptorLocations.end())
        return it->second;
    return std::nullopt;
}

std::optional<uint32_t>
Shader::GetDescriptorSize(const std::string &name) const {
    if (auto it = descriptorSizes.find(name); it != descriptorSizes.end())
        return it->second;
    return std::nullopt;
}

std::optional<Shader::Uniform>
...
std::optional<Shader::UniformBlock>
Shader::GetUniformBlock(const std::string &name) const {
    if (auto it = uniformBlocks.find(name); it != uniformBlocks.end())
        return it->second;
    return std::nullopt;
}

std::optional<Shader::Attribute>
Shader::GetAttribute(const std::string &name) const {
...
std::optional<VkDescriptorType>
Shader::GetDescriptorType(uint32_t location) const {
...
Shader::GetShaderStage(const std::filesystem::path &filename) {
...
VkShaderModule Shader::CreateShaderModule(
    const std::filesystem::path &moduleName, const std::string &moduleCode,
    const std::string &preamble, VkShaderStageFlags moduleFlag) {
    ...
    shader.setStringsWithLengthsAndNames(&shaderSource, nullptr,
                                         &shaderNameCstr, 1);
    ...
    if (!program.link(messages) || !program.mapIO()) {
        W_ERROR("Error while linking shader program.\n");
    }
    ...
    GlslangToSpv(*program.getIntermediate(static_cast<EShLanguage>(language)),
    ...
    Graphics::CheckVk(vkCreateShaderModule(
        *logicalDevice, &shaderModuleCreateInfo, nullptr, &shaderModule));
    return shaderModule;
}
...
void Shader::CreateReflection() {
        ...
        case UniformBlock::Type::Uniform:
            ...
            descriptorSetLayouts.emplace_back(
                UniformBuffer::GetDescriptorSetLayout(
                    static_cast<uint32_t>(uniformBlock.binding), descriptorType,
                    uniformBlock.stageFlags, 1));
            break;
        case UniformBlock::Type::Storage:
            descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
            descriptorSetLayouts.emplace_back(
                StorageBuffer::GetDescriptorSetLayout(
                    static_cast<uint32_t>(uniformBlock.binding), descriptorType,
                    uniformBlock.stageFlags, 1));
            break;
    ...
    for (const auto &[uniformName, uniform] : uniforms) {
        ...
        case 0x9055: // GL_IMAGE_2D_MULTISAMPLE
            descriptorType = uniform.writeOnly
                                 ? VK_DESCRIPTOR_TYPE_STORAGE_IMAGE
                                 : VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
            descriptorSetLayouts.emplace_back(Image2d::GetDescriptorSetLayout(
                static_cast<uint32_t>(uniform.binding), descriptorType,
                uniform.stageFlags, 1));
            break;
        ...
        case 0x9054: // GL_IMAGE_CUBE_MAP_ARRAY
            descriptorType = uniform.writeOnly
                                 ? VK_DESCRIPTOR_TYPE_STORAGE_IMAGE
                                 : VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
            descriptorSetLayouts.emplace_back(ImageCube::GetDescriptorSetLayout(
                static_cast<uint32_t>(uniform.binding), descriptorType,
                uniform.stageFlags, 1));
            break;
        default:
...
void Shader::IncrementDescriptorPool(
    ...
    if (auto it = descriptorPoolCounts.find(type);
        it != descriptorPoolCounts.end())
        it->second++;
    else
        descriptorPoolCounts.emplace(type, 1);
}

void Shader::LoadUniformBlock(const glslang::TProgram &program,
                              VkShaderStageFlags stageFlag, int32_t i) {
    ...
    uniformBlocks.emplace(reflection.name,
                          UniformBlock(reflection.getBinding(), reflection.size,
                                       stageFlag, type));
}

void Shader::LoadUniform(const glslang::TProgram &program,
                         VkShaderStageFlags stageFlag, int32_t i) {
        ...
        if (splitName.size() > 1) {
            for (auto &[uniformBlockName, uniformBlock] : uniformBlocks) {
                if (uniformBlockName == splitName.at(0)) {
                    uniformBlock.uniforms.emplace(
                        String::ReplaceFirst(reflection.name,
                                             splitName.at(0) + ".", ""),
                        Uniform(reflection.getBinding(), reflection.offset,
                                ComputeSize(reflection.getType()),
                                reflection.glDefineType, false, false,
                                stageFlag));
                    return;
                }
            }
    ...
    uniforms.emplace(reflection.name,
                     Uniform(reflection.getBinding(), reflection.offset, -1,
                             reflection.glDefineType, qualifier.readonly,
                             qualifier.writeonly, stageFlag));
}

void Shader::LoadAttribute(const glslang::TProgram &program,
                           VkShaderStageFlags stageFlag, int32_t i) {
    ...
    attributes.emplace(reflection.name,
                       Attribute(qualifier.layoutSet, qualifier.layoutLocation,
                                 ComputeSize(reflection.getType()),
                                 reflection.glDefineType));
}

int32_t Shader::ComputeSize(const glslang::TType *ttype) {
    // TODO: glslang::TType::computeNumComponents is available but has many
    // issues resolved in this method.
