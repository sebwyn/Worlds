...
RenderStage::RenderStage(std::vector<Attachment> images,
    ...
    : attachments(std::move(images)), subpasses(std::move(subpasses)),
      viewport(viewport), subpassAttachmentCount(this->subpasses.size()),
    ...
    for (const auto &image : attachments) {
        ...
        case Attachment::Type::Image:
            clearValue.color = {
                {image.GetClearColour().r, image.GetClearColour().g,
...
void RenderStage::Rebuild(const Swapchain &swapchain) {
    ...
    if (depthAttachment)
        depthStencil = std::make_unique<ImageDepth>(
            renderArea.GetExtent(), depthAttachment->IsMultisampled()
                                        ? msaaSamples
    ...
    framebuffers = std::make_unique<Framebuffers>(renderArea.GetExtent(), *this,
                                                  *renderpass, swapchain,
                                                  *depthStencil, msaaSamples);
    outOfDate = false;
    ...
    for (const auto &image : attachments) {
        if (image.GetType() == Attachment::Type::Depth)
            ...
            descriptors.emplace(image.GetName(), framebuffers->GetAttachment(
                                                     image.GetBinding()));
    }
