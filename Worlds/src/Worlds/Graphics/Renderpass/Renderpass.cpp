...
Renderpass::Renderpass(const RenderStage &renderStage, VkFormat depthFormat,
    ...
    for (const auto &attachment : renderStage.GetAttachments()) {
        auto attachmentSamples =
            attachment.IsMultisampled() ? samples : VK_SAMPLE_COUNT_1_BIT;

        VkAttachmentDescription attachmentDescription = {};
        attachmentDescription.samples = attachmentSamples;
        attachmentDescription.loadOp =
            VK_ATTACHMENT_LOAD_OP_CLEAR; // Clear at beginning of the render
                                         // pass.
        attachmentDescription.storeOp =
            VK_ATTACHMENT_STORE_OP_STORE; // // The image can be read from so
                                          // it's important to store the
                                          // attachment results
        attachmentDescription.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        attachmentDescription.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
        attachmentDescription.initialLayout =
            VK_IMAGE_LAYOUT_UNDEFINED; // We don't care about initial layout of
                                       // the attachment.
    ...
    for (const auto &subpassType : renderStage.GetSubpasses()) {
        ...
        for (const auto &attachmentBinding :
            ...
            if (!attachment) {
                W_ERROR("Failed to find a renderpass attachment bound to: ",
                           attachmentBinding, '\n');
                continue;
            }
        ...
        subpasses.emplace_back(std::make_unique<SubpassDescription>(
            VK_PIPELINE_BIND_POINT_GRAPHICS, subpassColourAttachments,
            depthAttachment));
    ...
    Graphics::CheckVk(vkCreateRenderPass(*logicalDevice, &renderPassCreateInfo,
                                         nullptr, &renderpass));
}
