...
Swapchain::Swapchain(const VkExtent2D &extent, const Swapchain *oldSwapchain)
    : extent(extent), presentMode(VK_PRESENT_MODE_FIFO_KHR),
    ...
    Graphics::CheckVk(vkCreateSwapchainKHR(*logicalDevice, &swapchainCreateInfo,
                                           nullptr, &swapchain));

    Graphics::CheckVk(vkGetSwapchainImagesKHR(*logicalDevice, swapchain,
                                              &imageCount, nullptr));
    ...
    for (uint32_t i = 0; i < imageCount; i++) {
        Image::CreateImageView(images.at(i), imageViews.at(i),
...
VkResult
Swapchain::AcquireNextImage(const VkSemaphore &presentCompleteSemaphore,
                            VkFence fence) {
    auto logicalDevice = Graphics::Get()->GetLogicalDevice();

    if (fence != VK_NULL_HANDLE)
        Graphics::CheckVk(
            vkWaitForFences(*logicalDevice, 1, &fence, VK_TRUE,
                            std::numeric_limits<uint64_t>::max()));

    auto acquireResult = vkAcquireNextImageKHR(
        *logicalDevice, swapchain, std::numeric_limits<uint64_t>::max(),
    ...
    // Graphics::CheckVk(vkWaitForFences(*logicalDevice, 1, &fenceImage,
...
VkResult Swapchain::QueuePresent(const VkQueue &presentQueue,
