...
class Swapchain {
  public:
    ...
    /**
     * Acquires the next image in the swapchain into the internal acquired
     * image. The function will always wait until the next image has been
     * acquired by setting timeout to UINT64_MAX.
     * @param presentCompleteSemaphore A optional semaphore that is signaled
     * when the image is ready for use.
     * @param fence A optional fence that is signaled once the previous command
    ...
    /**
     * Queue an image for presentation using the internal acquired image for
     * queue presentation.
     * @param presentQueue Presentation queue for presenting the image.
     * @param waitSemaphore A optional semaphore that is waited on before the
     * image is presented.
    ...
    bool IsSameExtent(const VkExtent2D &extent2D) {
        return extent.width == extent2D.width &&
