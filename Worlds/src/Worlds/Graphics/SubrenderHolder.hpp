...
namespace Worlds {
/**
 * @brief Class that contains and manages subrenders registered to a render
 * manager.
 */
class SubrenderHolder {
  ...
  public:
    ...
    template <typename T> T *Get() const {
        ...
        if (auto it = subrenders.find(typeId);
            it != subrenders.end() && it->second)
            return static_cast<T *>(it->second.get());

        // throw std::runtime_error("Subrender Holder does not have requested
        // Subrender");
        return nullptr;
    }
