...
std::vector<uint32_t> Model::GetIndices(std::size_t offset) const {
    ...
    vkCmdCopyBuffer(commandBuffer, indexBuffer->GetBuffer(),
    ...
    for (uint32_t i = 0; i < indexCount; i++) {
        std::memcpy(&indices[i],
                    static_cast<char *>(indicesMemory) + (i * sizeOfSrcT) +
                        offset,
                    sizeof(uint32_t));
    }
...
void Model::SetIndices(const std::vector<uint32_t> &indices) {
    ...
    VkBufferCopy copyRegion = {};
    copyRegion.size = indexStaging.GetSize();
    vkCmdCopyBuffer(commandBuffer, indexStaging.GetBuffer(),
                    indexBuffer->GetBuffer(), 1, &copyRegion);
