...
class String {
  public:
     ...
     */
    static std::string ReplaceAll(std::string str, std::string_view token,
                                  std::string_view to);
     ...
     * @return The string with the tokens replaced.
     */
    static std::string ReplaceFirst(std::string str, std::string_view token,
                                    std::string_view to);
     ...
     */
    template <typename T> static std::string To(T val) {
    ...
    // key is the data to hash, len is the size of the data (or how much of it
    // to hash against) code license: public domain or equivalent post:
    // https://notes.underscorediscovery.com/constexpr-fnv1a/

    static constexpr uint32_t
    fnv1a_32(std::string_view str, const uint32_t value = 0x811c9dc5) noexcept {
        return str.size() == 0
                   ? value
                   : fnv1a_32(std::string_view(str.data() + 1, str.size() - 1),
                              (value ^ uint32_t(str[0])) * 0x1000193);
    }

    static constexpr uint64_t
    fnv1a_64(std::string_view str,
             const uint64_t value = 0xcbf29ce484222325) noexcept {
        return str.size() == 0
                   ? value
                   : fnv1a_64(std::string_view(str.data() + 1, str.size() - 1),
